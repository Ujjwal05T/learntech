...
const ViewSwitcher = ({ currentView, onToggle }: { currentView: string; onToggle: (view: string) => void }) => {
  return (
    <div className="relative min-h-[80vh] w-full bg-gradient-to-br from-indigo-900 via-gray-900 to-slate-900 flex flex-col items-center justify-center overflow-hidden">
      ...
      >
        <button
          ...
          className={`group px-8 py-4 rounded-xl flex items-center gap-3 transition-all duration-300 transform hover:scale-105 ${
            currentView === "student"
              ? "bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg shadow-indigo-500/25"
              : "bg-white/10 text-white hover:bg-white/20 backdrop-blur-sm"
          }`}>
          <Users size={24} className={`transition-colors ${currentView === "student" ? "text-white" : "text-indigo-400 group-hover:text-white"}`} />
        ...
        <button
          ...
          <Building2 size={24} className={`transition-colors ${currentView === "company" ? "text-white" : "text-indigo-400 group-hover:text-white"}`} />
...
const CompanyView = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-slate-800 relative overflow-x-hidden">
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        ...
        <section className="flex items-center justify-center min-h-[480px] py-16">
          <div className="w-full max-w-5xl bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl p-12 mb-20 mx-auto">
            <div className="grid md:grid-cols-2 gap-16 items-center">
              <div>
                <h2 className="text-3xl font-bold text-white mb-6 flex items-center gap-3 justify-center md:justify-start">
                ...
                <ul className="space-y-5">
                  ...
                  */}
                  {["Traditional hiring may take months and costs $$$", "Resumes don't show real coding abilities", "Hard to find passionate, motivated developers"].map((text, idx) => (
                    <motion.li
                      key={idx}
                      whileHover={{ scale: 1.06, backgroundColor: '#ef4444', color: '#fff', x: 8 }}
                      transition={{ type: 'spring', stiffness: 300 }}
                      className="flex items-center gap-3 px-5 py-4 rounded-xl bg-gray-700/60 text-gray-200 shadow-md cursor-pointer hover:shadow-lg hover:text-white transition-all group"
                    >
                ...
                <ul className="space-y-5">
                  ...
                  */}
                  {["See developers build real projects in real time hackathons", "Access to pre-vetted, passionate talent", "Faster hiring with portfolio-backed candidates"].map((text, idx) => (
                    <motion.li
                      key={idx}
                      whileHover={{ scale: 1.06, backgroundColor: '#22c55e', color: '#fff', x: 8 }}
                      transition={{ type: 'spring', stiffness: 300 }}
                      className="flex items-center gap-3 px-5 py-4 rounded-xl bg-gray-700/60 text-gray-200 shadow-md cursor-pointer hover:shadow-lg hover:text-white transition-all group"
                    >
